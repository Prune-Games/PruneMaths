...
namespace Maths
{
    ...
    namespace Util
    {
        ...
        inline PRUNE_API f32 ToRadians(f32 in);

        // Return the given angular value in radians converted to degrees
        inline PRUNE_API f32 ToDegrees(f32 in);

        inline PRUNE_API f32 Clamp(f32 in, f32 min = 0.0f, f32 max = 1.0f);

        inline PRUNE_API Vec2 Clamp(Vec2 in, f32 min = 0.0f, f32 max = 1.0f);

        inline PRUNE_API Vec3 Clamp(Vec3 in, f32 min = 0.0f, f32 max = 1.0f);

        inline PRUNE_API Vec4 Clamp(Vec4 in, f32 min = 0.0f, f32 max = 1.0f);

        inline PRUNE_API f32 Abs(f32 in);

        inline PRUNE_API Vec2 Abs(Vec2 in);

        inline PRUNE_API Vec3 Abs(Vec3 in);

        inline PRUNE_API Vec4 Abs(Vec4 in);

        inline PRUNE_API s32 IClamp(s32 in, s32 min, s32 max);

        inline PRUNE_API u32 UClamp(u32 in, u32 min, u32 max);

        inline PRUNE_API f32 Lerp(f32 a, f32 b, f32 delta);

        inline PRUNE_API Vec3 Lerp(Vec3 a, Vec3 b, f32 delta);

        inline PRUNE_API f32 Mod(f32 in, f32 value);

        inline PRUNE_API Vec2 Mod(Vec2 in, f32 value);

        inline PRUNE_API Vec3 Mod(Vec3 in, f32 value);

        inline PRUNE_API s32 IMod(s32 in, s32 value);

        inline PRUNE_API f32 MinF(f32 a, f32 b);

        inline PRUNE_API f32 MaxF(f32 a, f32 b);

        inline PRUNE_API Vec3 MinV(Vec3 a, Vec3 b);

        inline PRUNE_API Vec3 MaxV(Vec3 a, Vec3 b);

        inline PRUNE_API s32 MinI(s32 a, s32 b);

        inline PRUNE_API s32 MaxI(s32 a, s32 b);

        // Smooth min function
        inline PRUNE_API f32 SMin(f32 a, f32 b, f32 delta);

        inline PRUNE_API bool IsNear(f32 a, f32 b, f32 prec = 0.0001f);

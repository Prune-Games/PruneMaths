...
#ifdef Prune_EngineDLL
#define PRUNE_API __declspec(dllexport)
#else
#define PRUNE_API
#endif
...
namespace Maths
{
    ...
    namespace Util
    {
        ...
        CUDA_FUNCTION inline PRUNE_API f32 ToRadians(f32 in);

        // Return the given angular value in radians converted to degrees
        CUDA_FUNCTION inline PRUNE_API f32 ToDegrees(f32 in);

        CUDA_FUNCTION inline PRUNE_API f32 Clamp(f32 in, f32 min = 0.0f, f32 max = 1.0f);

        CUDA_FUNCTION inline PRUNE_API Vec2 Clamp(Vec2 in, f32 min = 0.0f, f32 max = 1.0f);

        CUDA_FUNCTION inline PRUNE_API Vec3 Clamp(Vec3 in, f32 min = 0.0f, f32 max = 1.0f);

        CUDA_FUNCTION inline PRUNE_API Vec4 Clamp(Vec4 in, f32 min = 0.0f, f32 max = 1.0f);

        CUDA_FUNCTION inline PRUNE_API f32 Abs(f32 in);

        CUDA_FUNCTION inline PRUNE_API Vec2 Abs(Vec2 in);

        CUDA_FUNCTION inline PRUNE_API Vec3 Abs(Vec3 in);

        CUDA_FUNCTION inline PRUNE_API Vec4 Abs(Vec4 in);

        CUDA_FUNCTION inline PRUNE_API s32 IClamp(s32 in, s32 min, s32 max);

        CUDA_FUNCTION inline PRUNE_API u32 UClamp(u32 in, u32 min, u32 max);

        CUDA_FUNCTION inline PRUNE_API f32 Lerp(f32 a, f32 b, f32 delta);

        CUDA_FUNCTION inline PRUNE_API Vec3 Lerp(Vec3 a, Vec3 b, f32 delta);

        CUDA_FUNCTION inline PRUNE_API f32 Mod(f32 in, f32 value);

        CUDA_FUNCTION inline PRUNE_API Vec2 Mod(Vec2 in, f32 value);

        CUDA_FUNCTION inline PRUNE_API Vec3 Mod(Vec3 in, f32 value);

        CUDA_FUNCTION inline PRUNE_API s32 IMod(s32 in, s32 value);

        CUDA_FUNCTION inline PRUNE_API f32 MinF(f32 a, f32 b);

        CUDA_FUNCTION inline PRUNE_API f32 MaxF(f32 a, f32 b);

        CUDA_FUNCTION inline PRUNE_API Vec3 MinV(Vec3 a, Vec3 b);

        CUDA_FUNCTION inline PRUNE_API Vec3 MaxV(Vec3 a, Vec3 b);

        CUDA_FUNCTION inline PRUNE_API s32 MinI(s32 a, s32 b);

        CUDA_FUNCTION inline PRUNE_API s32 MaxI(s32 a, s32 b);

        // Smooth min function
        CUDA_FUNCTION inline PRUNE_API f32 SMin(f32 a, f32 b, f32 delta);

        CUDA_FUNCTION inline PRUNE_API bool IsNear(f32 a, f32 b, f32 prec = 0.0001f);

        // Returns a string with the hex representation of number
        // TODO Test parity with big/little endian
        inline PRUNE_API std::string GetHex(u64 number);

        // Fills the given buffer with the hex representation of number
        // WARNING: buffer must be at least 16 char long
        // TODO Test parity with big/little endian
        inline PRUNE_API void GetHex(char* buffer, u64 number);

        inline PRUNE_API u64 ReadHex(const std::string& input);

        // Set of functions used to generate some shapes
        // TODO is this still relevant ?

        void PRUNE_API GenerateSphere(s32 x, s32 y, std::vector<Vec3>* PosOut, std::vector<Vec3>* NormOut, std::vector<Vec2>* UVOut);

        void PRUNE_API GenerateCube(std::vector<Vec3>* PosOut, std::vector<Vec3>* NormOut, std::vector<Vec2>* UVOut);

        void PRUNE_API GenerateDome(s32 x, s32 y, bool reversed, std::vector<Vec3>* PosOut, std::vector<Vec3>* NormOut, std::vector<Vec2>* UVOut);

        void PRUNE_API GenerateCylinder(s32 x, s32 y, std::vector<Vec3>* PosOut, std::vector<Vec3>* NormOut, std::vector<Vec2>* UVOut);

        void PRUNE_API GeneratePlane(std::vector<Vec3>* PosOut, std::vector<Vec3>* NormOut, std::vector<Vec2>* UVOut);

        void PRUNE_API GenerateSkyPlane(std::vector<Vec3>* PosOut, std::vector<Vec3>* NormOut, std::vector<Vec2>* UVOut);

        Vec3 PRUNE_API GetSphericalCoord(f32 longitude, f32 latitude);
    };
}
